...
# source("functions_coloc_signals_adj.R")
# source("functions_get_genomatrix.R")
# source("functions_plot_locus.R")

## Function for running coloc and making locuscompare and/or locuszoom figures
                      ...
                      qtl_trait = "qtl", gwas_trait = "gwas")
{
  ...
  ## Genotype data ----
  ### Might need to exclude monomorphic variants from df `select`
  chr <- select$chr[1]
  ...
  if (qtl_coloc_mode == "single" & gwas_coloc_mode == "single") {
    ...
    ## Using modified coloc.signals function to allow to use finemap.signals() with mode = mask using pvalues
    cat(paste0("Running coloc.signals in ", coloc_mode, " mode"), fill = TRUE)
    cat("df1:", names(dat_1), "=", dat_1[["method"]], fill = T)
    cat("df2:", names(dat_2), "=", dat_2[["method"]], fill = T)

    # Independent signals in teh GWAS dataset
    finemapped_signals <- finemap.signals_adj(D = dat_1, LD = ld_r, method = dat_1[["method"]],
                                              maxhits = maxhits, r2thr = 0.01, pthr = pthr)
    cat("Independent signals in dat_1", fill = T)
    ...
    stopifnot(names(finemapped_signals) %in% result$hit1)
    result <- result[match(names(finemapped_signals), result$hit1), ]

    # No locusplot figures if using coloc-condmask, and only one independent signal or show no ld
    if (nrow(result) == 1 & !(ld_show)) {
      locuscompare <- FALSE
      locuszoom <- FALSE
    }
  }
  ...
  if (locuscompare | locuszoom) {
    if (sum(!is.na(result[, "PP.H4.abf"])) > 0) { #result[1, "PP.H4.abf"] > 0.5 &
      ...
      if (gwas_hit_nearest_log10p_5 < 250000 & !(paste0("chr", chr) %in% "chr6" & pos_pheno > 28510120 & pos_pheno < 33480577)) {
        ...
        for (j in 1:min(nrow(result), 3)) {
          if (qtl_coloc_mode %in% c("cond", "mask") | gwas_coloc_mode %in% c("cond", "mask")) {
            ...
            idx <- which(colnames(result_plot) == paste0("z.df1.row", j))
            plot_data$gwas_pval <- pnorm(-abs(result_plot[,idx]))*2
            idx <- which(colnames(result_plot) == paste0("z.df2.row", j))
          ...
          g <- locusplot_ld(data = plot_data,
                            ...
                            highlight_index_gwas.label = plot_data[which.min(plot_data$gwas_pval), "gwas_rsid"],
                            highlight_index_qtl.label = plot_data[which.min(plot_data$qtl_pval), "qtl_rsid"],
          ...
          if ("locuscompare" %in% names(g)) {
            ...
            ggsave(filename = paste0(locuscompare_fig, ".", qtl_lead_snv, ".pp4_", round(result[j, "PP.H4.abf"], 2), file_name_end), plot = g[[idx]], width = 3.8, height = 4.35)
          }
          if ("locuszoom" %in% names(g)) {
            idx <- which(names(g) == "locuszoom")
            ggsave(filename = paste0(locuszoom_fig, ".", qtl_lead_snv, ".pp4_", round(result[j, "PP.H4.abf"], 2), file_name_end), plot = g[[idx]], width = 4, height = 5)
          }
        }
      }
    }
